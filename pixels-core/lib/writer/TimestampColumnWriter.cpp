...
int TimestampColumnWriter::write(std::shared_ptr<ColumnVector> vector, int size) {
    ...
    while ((curPixelIsNullIndex + nextPartLength) >= pixelStride) {
        int curPartLength = pixelStride - curPixelIsNullIndex;
...
void TimestampColumnWriter::writeCurPartTimestamp(std::shared_ptr<ColumnVector> columnVector,
...
void TimestampColumnWriter::newPixel() {
    if (runlengthEncoding) {
        ...
        auto curVecPartitionBuffer = std::make_shared<ByteBuffer>(curPixelVectorIndex * sizeof(int));
